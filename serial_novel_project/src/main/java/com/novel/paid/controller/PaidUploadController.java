...
@Log4j
@Controller
public class PaidUploadController {
	...
	// ■ 폼으로 파일 업로드하기
		...
		// ※ 05.17 일반 컨트롤러에서 Rest호출을 처리하고 싶다면? (@ResponseBody 사용). 코드 수정(교안13의 31)
			...
			@ResponseBody
			public ResponseEntity<List<PaidAttachVO>> uploadFormPost(MultipartFile[] uploadFile) {
				
				// ● 05.17 BoardAttachVO는 파일 한개, 현재 파일 업로드는 여러 파일을 동시에 업로드 해야하므로 List 사용
				List<PaidAttachVO> list = new ArrayList<>();
				...
				for(MultipartFile multipartFile : uploadFile) {
					...
					PaidAttachVO attachDTO = new PaidAttachVO();
	...
	// ■ ajax로 파일 업로드하기
		...
		@ResponseBody
		public ResponseEntity<List<PaidAttachVO>> uploadAjaxPost(MultipartFile[] uploadFile) {
